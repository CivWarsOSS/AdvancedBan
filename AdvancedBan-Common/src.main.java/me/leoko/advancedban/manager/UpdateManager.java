...
public class UpdateManager {
	...
	public void setup() {
		...
		if (!mi.contains(mi.getMessages(), "Tempipban")) {
			try {
				FileUtils.writeLines(mi.getMessagesFile(), Arrays.asList("", "Tempipban:",
						"  Usage: \"&cUsage &8\\xbb &7&o/tempipban [Name/IP] [Xmo/Xd/Xh/Xm/Xs/#TimeLayout] [Reason/@Layout]\"",
						"  MaxDuration: \"&cYou are not able to ban more than %MAX%sec\"", "  Layout:",
						"  - '%PREFIX% &7Temporarily banned'", "  - '&7'", "  - '&7'",
						"  - \"&cReason &8\\xbb &7%REASON%\"", "  - \"&cDuration &8\\xbb &7%DURATION%\"", "  - '&7'",
						"  - '&8Unban application in TS or forum'", "  - \"&eTS-Ip &8\\xbb &c&ncoming soon\"",
						"  - \"&eForum &8\\xbb &c&ncoming soon\"", "  Notification:",
						"  - \"&c&o%NAME% &7got banned by &e&o%OPERATOR%\"", "  - \"&7For the reason &o%REASON%\"",
						"  - \"&7&oThis player got banned for &e&o%DURATION%\"", "", "ChangeReason:",
		...
		try {
			...
			if (!mi.contains(mi.getConfig(), "Debug")) {
				lines.addAll(Arrays.asList("",
						...
						"# designed to find bugs.", "Debug: false"));
			}
			if (mi.contains(mi.getConfig(), "LogsPurgeDays")) {
				lines.removeAll(Arrays.asList("",
						...
						"# By default is set to 10 days.", "LogsPurgeDays: 10"));
			}
			if (!mi.contains(mi.getConfig(), "LogPurgeDays")) {
				lines.addAll(Arrays.asList("",
						"# This is the amount of days that we should keep plugin logs in the plugins/AdvancedBan/logs folder.",
						"# By default is set to 10 days.", "LogPurgeDays: 10"));
			}
			if (!mi.contains(mi.getConfig(), "DisablePrefix")) {
				lines.addAll(Arrays.asList("", "# Removes the prefix of the plugin in every message.",
						"DisablePrefix: false"));
			}
			FileUtils.writeLines(file, lines);
